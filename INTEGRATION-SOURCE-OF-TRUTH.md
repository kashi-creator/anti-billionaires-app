...
- **`/create-checkout-session` does NOT configure the 30-day Stripe trial. LAUNCH BLOCKER.** Surfaced 2026-05-02 by Phase 1 Step 3.5 informational check. `app.py:1348` calls `stripe.checkout.Session.create(...)` with `mode="subscription"` and a `line_items[{price: STRIPE_PRICE_ID}]`, but does NOT pass `subscription_data={"trial_period_days": 30}`. Every Stripe-path signup will be charged $99 on day 1, directly contradicting (a) the locked 30-day trial decision (Decisions Log 2026-05-02), (b) the new public FAQ #2 ("Your first 30 days are free"), and (c) Terms §2 ("trial-period months — no payment was made"). Material contract-law and refund-fraud exposure if any signup happens against this code. Two possible fixes — manager call: (i) add `subscription_data={"trial_period_days": 30}` in code (1 line, ships immediately, requires verifying `STRIPE_PRICE_ID` is configured for trial-eligible recurring billing); (ii) configure trial on the Stripe Price object itself (zero code change but requires Stripe dashboard work). Phase 1 did NOT touch this — out of scope, prompt explicitly said "informational, no code change." Tracked for whichever phase Kashi assigns. Note: this also implicitly means `User.subscription_status` will never be `"trialing"` (it'll go straight to `"active"`), so the post-Phase-0 fix to `has_active_subscription` (which accepts `"trialing"`) was technically defensive — not yet exercised in prod.
