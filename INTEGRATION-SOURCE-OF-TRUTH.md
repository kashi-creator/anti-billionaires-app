...
- **Email render fails silently in threaded sends when `_external=True` URLs are used.** Surfaced 2026-05-02 by Phase 0C executor. `email_send.py` renders templates inside a background thread that has app_context but NOT request_context. Templates (e.g. `lifetime_unlocked.html`) use `url_for(..., _external=True)` for absolute URLs in CTA buttons; without a request context AND without `SERVER_NAME` set in app config, this raises `RuntimeError`. Currently masked because the Phase 0C send is wrapped in try/except, AND the `.txt` template variant doesn't use `_external=True` so the stdout fallback in dev still prints. **In production with `RESEND_API_KEY` set, all emails that include `_external=True` URLs will silently fail to render the HTML body.** Fix: set `app.config["SERVER_NAME"] = os.environ.get("SERVER_NAME") or "anti-billionaires-app-production.up.railway.app"` in `app.py` startup. Caveat: setting `SERVER_NAME` affects cookie domain matching and blueprint URL generation — verify locally before pushing. Folded into Phase 1 scope (same threading-refactor blast radius as the GHL lift). Tracked.
