...
- **Events + RSVPs** — three event types (`chapter_recurring`, `weekly_recurring`, `member_meetup`; `official_one_off` reserved). Recurring events live as a "template" row + per-date "occurrence" child rows generated 8 weeks ahead, idempotent on every `/events` GET. Members can post local meetups; chapter / recurrence fields are admin-only (server-side gate).
